...
\begin{tikzpicture}[
  ...
  % Merge + evaluation
  \node[box, right=2.5cm of prep] (compare) {6. Compare\\Predictions};
  \node[box, right=of compare] (plot) {7. Plot\\Waveforms};
  \node[box, below=0.5cm of plot] (analyze) {8. Error\\Analysis};
...
\begin{frame}[fragile]{Step 8: Error Analysis (\texttt{error\_analysis/analyze.py})}

\textbf{Purpose:} Deep diagnostic analysis of model predictions beyond aggregate metrics.
...
# Auto-runs after run-all
python cli.py run-all
\end{lstlisting}

\smallskip
Loads models, scalers, and training data from working directory; reads experiment config.

\column{0.48\textwidth}
\textbf{Output:} \texttt{experiment/error\_analysis/}
\begin{itemize}
  \item 8 diagnostic plot files (.png)
  \item \texttt{error\_analysis\_report.json}
  \item \texttt{summary.pdf} (multi-page compilation)
\end{itemize}

\smallskip
Runs all 8 analyses, saves a JSON report with every computed metric, and compiles plots into a single PDF.
\end{columns}
\end{frame}

% ============================================================
\begin{frame}{Error Analysis: Diagnostics}

\begin{columns}[T]
\column{0.48\textwidth}
\textbf{Signal regression diagnostics:}
\begin{enumerate}
  \item \textbf{Per-slot MAE/RMSE} --- which signal positions are hardest to predict
  \item \textbf{Error vs count} --- box plots of errors grouped by true signal count
  \item \textbf{Temporal profile} --- error vs $t_0$ position in the time window
  \item \textbf{Amplitude dependency} --- error vs true amplitude by quartile
\end{enumerate}

\column{0.48\textwidth}
\textbf{Multi-signal \& count diagnostics:}
\begin{enumerate}
  \setcounter{enumi}{4}
  \item \textbf{Spacing analysis} --- error vs min inter-signal distance (overlap effects)
  \item \textbf{Worst cases} --- top-20 worst waveforms plotted in a 4$\times$5 grid
  \item \textbf{Count model} --- normalized confusion matrix, per-class accuracy, error distribution
  \item \textbf{Residual analysis} --- QQ plots, histograms with normal fit, Shapiro-Wilk test
\end{enumerate}
\end{columns}
\end{frame}
...
\begin{itemize}
  ...
  \item \texttt{error\_analysis/}: diagnostic plots, JSON report, summary PDF
\end{itemize}
